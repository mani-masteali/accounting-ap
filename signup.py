...
class User:
    ...
    def get_phone_number(self,phoneNumber):
        if len(phoneNumber)==11 and len(re.findall('[0-9]',phoneNumber))==len(phoneNumber) and  bool(re.search('^09.',phoneNumber))==True:
            ...
            raise ValueError(Console().print('[bold red]phone number must have eleven digits.'))
    ...
    def get_password(self,password):
        if len(password)>=6 and len(re.findall('[a-z]',password))>=1 and len(re.findall('[A-Z]',password))>=1 and len(re.findall("[!#$%&'()*+,-./:;<=>?@[\]^_`{|}~]",password))>=1 and len(re.findall('[0-9]',password))>=1:
            ...
            raise ValueError(Console().print('[bold red]password must be at least 6 characters long.'))
        elif len(re.findall('[a-z]',password))<1:
            raise ValueError(Console().print('[bold red]password must contain at least one lowercase letter.'))
        elif len(re.findall('[A-Z]',password))<1:
            raise ValueError(Console().print('[bold red]password must contain at least one uppercase letter.'))
